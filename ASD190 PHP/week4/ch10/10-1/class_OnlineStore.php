...
class OnlineStore
{
    ...
    {
        if (!$this->DBConnect->connect_error) $this->DBConnect->close();
    }
    ...
    public function getStoreInformation()
    {
        ...
        if ($this->storeID != "") {
            ...
            $QueryResult = $this->DBConnect->query($SQLString);
            if ($QueryResult != FALSE) $retval = $QueryResult->fetch_assoc();
        }
        return ($retval);
    }
    ...
    {
        if (!empty($_GET['ItemToAdd'])) $this->addItem();
        if (!empty($_GET['ItemToRemove'])) $this->removeItem();
        if (!empty($_GET['EmptyCart'])) $this->emptyCart();
    }

    private function addItem()
    {
        $ProdID = $_GET['ItemToAdd'];
        if (array_key_exists($ProdID, $this->shoppingCart)) $this->shoppingCart[$ProdID] += 1;
    }
    private function removeItem()
    {
        $ProdID = $_GET['ItemToRemove'];
        if (array_key_exists($ProdID, $this->shoppingCart)) if ($this->shoppingCart[$ProdID] > 0) $this->shoppingCart[$ProdID] -= 1;
    }
